...
const { EmbedBuilder, MessageFlags, ActionRowBuilder, ButtonBuilder, ButtonStyle, StringSelectMenuBuilder } = require('discord.js'); // ✅ נוספו ייבואים חסרים
 ...
 */
async function getDetailedInactivityStats(client) { // ✅ פונקציה זו הועברה ונוספה לכאן
    const allTracked = await db.collection('memberTracking').get();
    const now = Date.now();
    let inactive7Days = 0;
 ...
 */
function buildMainPanelEmbed(client, stats) { // ✅ פונקציה זו הועברה ונוספה לכאן
    return new EmbedBuilder()
 ...
 */
function buildMainPanelComponents() { // ✅ פונקציה זו הועברה ונוספה לכאן
